    ...
    await page.evaluate(async () => {
        ...
        await new Promise((resolve, reject) => {
            ...
            var timer = setInterval(() => {
                ...
                const totalItems = parseInt(document.querySelector('.prd-qty').innerHTML.replace(/[^\d]/g, ''))
                const callectedItems = document.querySelectorAll('.prd').length
                window.scrollBy(0, distance);
                totalHeight += distance;
                inc = inc + 1
                if (callectedItems>=totalItems  ) {
                    clearInterval(timer);
                    resolve();
                }
            }, 200);
